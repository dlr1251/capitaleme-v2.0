...
async function processBlogData(blogData, lang) {
  console.log(`[RUNTIME DEBUG] Processing ${blogData.length} blog posts for lang: ${lang}`);
  
  const allBlogPosts = blogData
    .filter(post => {
      // Filter by language and ensure required fields exist
      return post.lang === lang && post.title && post.published === true;
    })
    .map(post => {
      console.log(`[RUNTIME DEBUG] Processing blog post: ${post.title}`);
      return {
        id: post.id || post.notion_id,
        title: post.title || '',
        ...
        date: post.pub_date || post.date || post.last_edited || new Date().toISOString(),
        pubDate: post.pub_date || post.date || post.last_edited || new Date().toISOString(),
        href: `/${lang}/blog/${post.slug || post.title?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`,
        isFeatured: post.featured || false,
        lastEdited: post.last_edited || post.updated_at || new Date().toISOString(),
  ...
  console.log(`[RUNTIME DEBUG] Filtered to ${allBlogPosts.length} valid blog posts`);
