...
const BlogPostsList = ({ posts }: BlogPostsListProps) => {
  const [filters, setFilters] = useState<FilterState>({ author: '', category: '', startDate: '', endDate: '' });
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [searchTerm, setSearchTerm] = useState<string>('');
  const pageSize = 8;

  // Get unique categories and authors for filter dropdowns
  const categories = [...new Set(posts.map((post: Post) => post.data.category))].filter(Boolean);
  const authors = [...new Set(posts.map((post: Post) => post.data.author))].filter(Boolean);
