...
const BlogMegaMenu: React.FC<BlogMegaMenuProps> = ({ lang, menuData = {} }) => {
  ...
  useEffect(() => {
    if (menuData.latestNews && menuData.latestNews.length > 0) {
      ...
      const transformedPosts = menuData.latestNews.map((post: any) => ({
        ...post,
        author: {
          name: post.author || 'Capital M Law',
          avatar: '/images/team/danielluque.jpg', // Default avatar
          role: 'Legal Expert'
        },
  ...
  const getRelativeTime = useCallback((dateString: string): string => {
    ...
    if (diffInDays > 0) {
      if (diffInDays === 1) {
        return lang === 'en' ? '1 day ago' : 'hace 1 día';
      } else if (diffInDays < 7) {
        return lang === 'en' ? `${diffInDays} days ago` : `hace ${diffInDays} días`;
      } else if (diffInDays < 30) {
        const weeks = Math.floor(diffInDays / 7);
        return lang === 'en' ? `${weeks} week${weeks > 1 ? 's' : ''} ago` : `hace ${weeks} semana${weeks > 1 ? 's' : ''}`;
      } else {
        const months = Math.floor(diffInDays / 30);
        return lang === 'en' ? `${months} month${months > 1 ? 's' : ''} ago` : `hace ${months} mes${months > 1 ? 'es' : ''}`;
      }
    } else if (diffInHours > 0) {
      return lang === 'en' ? `${diffInHours} hour${diffInHours > 1 ? 's' : ''} ago` : `hace ${diffInHours} hora${diffInHours > 1 ? 's' : ''}`;
    } else if (diffInMinutes > 0) {
      return lang === 'en' ? `${diffInMinutes} minute${diffInMinutes > 1 ? 's' : ''} ago` : `hace ${diffInMinutes} minuto${diffInMinutes > 1 ? 's' : ''}`;
    } else {
      return lang === 'en' ? 'Just now' : 'Ahora mismo';
    }
  }, [lang]);

  // Format date for the beautiful overlay
  const formatDateForOverlay = useCallback((dateString: string) => {
    const date = new Date(dateString);
  ...
  const getExcerpt = useCallback((text: string, wordLimit: number = 15) => {
    if (!text) return '';
                    ...
                    {latestBlogPosts[0].createdDate && (
                      <div className="absolute top-2 left-2">
                        <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-white/90 text-gray-700 shadow-sm">
                          📅 {formatDateForOverlay(latestBlogPosts[0].createdDate).day} {formatDateForOverlay(latestBlogPosts[0].createdDate).month}
                        </span>
                      </div>
                    )}
