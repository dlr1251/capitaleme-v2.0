...
import { HiBookOpen, HiFilter, HiExternalLink, HiDocumentText } from 'react-icons/hi';
...
const CLKRRepository = ({ lang = 'en' }: CLKRRepositoryProps) => {
  ...
  useEffect(() => {
    const fetchCLKRData = async () => {
      try {
        ...
        setClkrServices([
          {
            title: 'Real Estate vs Business Visa',
            href: `/${lang}/clkr/real-estate-vs-business-visa`,
            description: 'Understanding the differences between real estate and business visa requirements',
            module: 'Real Estate & Urbanism'
          },
          {
            title: 'Property Registration Process',
            href: `/${lang}/clkr/property-registration-process`,
            description: 'Step-by-step guide to registering property in Colombia',
            module: 'Real Estate & Urbanism'
          },
          {
            title: 'Foreign Investment in Real Estate',
            href: `/${lang}/clkr/foreign-investment-real-estate`,
            description: 'Legal framework for foreign investment in Colombian real estate',
            module: 'Real Estate & Urbanism'
          },
          {
            title: 'Cédula de Extranjería',
            href: `/${lang}/clkr/cedula-extranjeria`,
            description: 'Foreigner ID card process and requirements',
            module: 'Immigration Services'
          },
          {
            title: 'Salvoconducto',
            href: `/${lang}/clkr/salvoconducto`,
            description: 'Safe conduct permit for foreigners in Colombia',
            module: 'Immigration Services'
          },
          {
            title: 'Beneficiary Visa',
            href: `/${lang}/clkr/beneficiary-visa`,
            description: 'Visa process for family members of residents',
            module: 'Immigration Services'
          }
        ]);
        setModules(['Real Estate & Urbanism', 'Immigration Services', 'Legal Services']);
      } finally {
        setIsLoading(false);
      }
    };
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 bg-${moduleColor}-100 rounded-lg flex items-center justify-center group-hover:bg-${moduleColor}-200 transition-colors duration-300`}>
                      <HiDocumentText className={`w-5 h-5 text-${moduleColor}-600`} />
                    </div>
                    <div>
                  ...
                  <HiExternalLink className="w-4 h-4 text-gray-400 group-hover:text-purple-600 transition-colors duration-300" />
                </div>
