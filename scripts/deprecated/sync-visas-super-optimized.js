...
// Configuration
const BATCH_SIZE = 5; // Process 5 visas simultaneously
...
async function extractVisaDataOptimized(visa, notion) {
  try {
    const properties = visa.properties;
    
    // Extract basic properties efficiently - using the same property names as original
    ...
    // For now, skip content fetching to optimize performance
...
async function processVisaBatch(visas, notion) {
  const results = await Promise.allSettled(
    visas.map(async (visa) => {
      try {
        const visaData = await extractVisaDataOptimized(visa, notion);
...
async function main() {
  console.log('🚀 Starting SUPER OPTIMIZED visa sync from Notion to Supabase...');
  ...
  console.log('\n⏱️  Starting super optimized sync...');

  try {
    const result = await syncVisasSuperOptimized();
    ...
    console.log(`   - Performance: ${(result.totalProcessed / duration).toFixed(2)} visas/second`);
    ...
    console.error('❌ SUPER OPTIMIZED Visa sync failed:', error);
    process.exit(1);
  }
}
